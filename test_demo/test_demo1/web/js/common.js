    ...
    actionDiv.dialog({
        ...
        onOpen: function() {
            $(this).dialog('move', {
                ...
                left: ($(window).width() - $(this).panel('panel').outerWidth()) / 2 + $(document).scrollLeft()
            });
        }
    });
    dialogDiv = $('<div id="dlg"></div>');
    dialogFrame = $('<iframe id="dialogFrame" src="" frameborder="0" width="100%" height="100%" />');
    dialogDiv.append("<div id=\"bb\" class=\"text-center\">\n\
        <a href=\"#\" class=\"easyui-linkbutton btn-confirm l-btn l-btn-small\" onclick=\"ok();\"><span class=\"l-btn-left\"><span class=\"l-btn-text\">确 定</span></span></a>\n\
        <a href=\"#\" class=\"easyui-linkbutton l-btn l-btn-small\" onclick=\"$('#dlg').dialog('close')\"><span class=\"l-btn-left\"><span class=\"l-btn-text\">取 消</span></span></a>\n\
        </div>");
...
//            $.messager.alert('确认', getConfirmAlert(actName, objName), 'question', function() {
//                $.post(curr_actionUrl, {}, function(result) {
//                    if (result == "success") {
//                        $(gridName).datagrid("reload");
//                        if (curr_callback != null) {
...
$(document).ready(function() {
    ...
    ;
    //    document.addCookie("operationTime="+ escape (new Date().getTime())); 
    document.cookie = "operationTime=" + escape(new Date().getTime());
    // index.jsp page move to top where click <a href='#' onclick=''> ,
    $("a[href='#'][onclick]").attr("href", "javascript:void(0);");
});
...
/**
 * 校验宽度：500 、 600 、800，800不够请调用openFullScreen打开全屏
 */
...
/**
 * 未选择或者选择了多个返回 undefined ，
 * 否则，不需要返回字段时返回true，需要返回字段时返回对应字段值
 * 需要替换MVC模式uri中的{id}
 */
function checkSelectOne(gridName, actName, objName, field) {
    var rows = $(gridName).datagrid('getSelections');
    if (rows.length == 0) {
        $.messager.alert("提示:", getSelectSomeAlert(actName, objName), "warning");
...
/**
 * 未选择记录返回 undefined ，
 * 否则，不需要返回字段时返回true，需要返回字段时返回对应字段值的数组
 */
function checkSelectSome(gridName, actName, objName, field) {
    var rows = $(gridName).datagrid('getSelections');
    if (rows.length == 0) {
        $.messager.alert("提示:", getSelectSomeAlert(actName, objName), "warning");
    } else {
        if (field) {
            if (!rows[0][field]) {
                return undefined;//字段未定义，直接返回undefined
            }
            var arrId = [];
            for (var i = 0; i < rows.length; i++) {
                arrId.push(rows[i][field]);
            }
            return arrId;
        }
        return true;
...
//        }
    ...
    url = url.replace("common.js", "jquery.easyui.validType.js");
